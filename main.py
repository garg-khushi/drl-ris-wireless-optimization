...
# main.py
import gym
from gym.envs.registration import register  # Critical import
import environment  # Your custom environment module


register(
    id='RISEnv-v0',
    entry_point='environment:RIS_MISO_PDA',    # Points to class in environment.py
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", default="RISEnv-v0", help='Environment name')
    ...
    parser.add_argument('--lambda_imp', type=float, default=0.1, help='Impulse occurrence probability')
    parser.add_argument('--sigma1', type=float, default=0.5, help='Std dev of impulsive noise component')
    ...
    if args.policy == "Beta_Space_Exp_SAC":
        file_name = f"Beta_Space_Exp_SAC_{args.objective_function}_{args.seed}"
    else:
        file_name = f"sac_{args.objective_function}_{args.seed}"
    #save_path = f"Beta_min. = {args.beta_min}, K = {args.num_users}, M = {args.num_antennas}, N = {args.num_RIS_elements}, P_t = {float(args.power_t)}"
    save_path = f"beta_min={args.beta_min},k={args.num_users},m={args.num_antennas},l={args.num_RIS_elements},power={int(args.power_t)}"
    ...
    for t in range(int(args.max_time_steps)):
        ...
        if (t + 1) % 100 == 0:
            ...
            print(f"Time step: {t + 1} Max. Reward: {float(max_reward):.3f} Max. Mismatch Reward: {float(max_mismatch_reward):.3f}")
